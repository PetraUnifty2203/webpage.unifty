...
class plgSystemQuix extends JPlugin
{
    ...
    public function onAfterRoute()
    {
        ...
        if ($this->app->isSite() && JFile::exists(JPATH_SITE.'/components/com_quix/helpers/theme.php')) {
    ...
    */
    public static function addQuixTrapCSSfrontend()
    {
        ...
        /*
         * Add bundle to reduce multiple css files
         * @since 2.7.7
         */
        $config     = JComponentHelper::getParams('com_quix');
        $reduceHttp = $config->get('reduceHttp', '1');

        if ($reduceHttp === '1') {
            $bundle = JUri::root(true).'/libraries/quix/assets/css/quix-bundle.css?'.$version;
            $document->addHeadLink($bundle, 'stylesheet', 'rel', $attr);
        } else {
            $quixBS   = JUri::root(true).'/libraries/quix/assets/css/qxbs.css?'.$version;
            $quixKit  = JUri::root(true).'/libraries/quix/assets/css/qxkit.css?'.$version;
            $quixCSS  = JUri::root(true).'/libraries/quix/assets/css/quix.css?'.$version;
            ...
            $document->addHeadLink($quixBS, 'stylesheet', 'rel', $attr);
            $document->addHeadLink($quixKit, 'stylesheet', 'rel', $attr);
            $document->addHeadLink($quixCSS, 'stylesheet', 'rel', $attr);
            $document->addHeadLink($quixIcon, 'stylesheet', 'rel', $attr);
        }
    }
